...
Usage:
    python -m ceu_crawler.processing.cli process [--limit N] [--provider NAME]
    python -m ceu_crawler.processing.cli stats
    python -m ceu_crawler.processing.cli reprocess --id RECORD_ID
"""
import argparse
import logging
import sys
...
def setup_logging(verbose: bool = False, debug: bool = False):
    ...
    for module in ['ceu_crawler.processing', 'ceu_crawler.processing.processor',
                   'ceu_crawler.processing.nlp_processor', 'ceu_crawler.processing.text_extractor',
                   'ceu_crawler.processing.ceu_patterns']:
...
Examples:
  Process pending records:
    python -m ceu_crawler.processing.cli process --limit 50

  Process for specific provider:
    python -m ceu_crawler.processing.cli process --provider pesi

  Show statistics:
    python -m ceu_crawler.processing.cli stats

  Reprocess a specific record:
    python -m ceu_crawler.processing.cli reprocess --id abc123

  Test extraction on HTML file:
    python -m ceu_crawler.processing.cli test --html-file page.html

  Test extraction on URL:
    python -m ceu_crawler.processing.cli test --url https://example.com/course
        """
    )
